    ...
    <main className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            <div className="font-extrabold text-violet-700">Cantus Dei</div>
            <div className="text-sm text-slate-500">Olá, {user?.nome}</div>
          </div>
          <button
      ...
      <section className="max-w-6xl mx-auto p-6 sm:py-10">
        <div className="flex items-center justify-between gap-4 mb-7">
          <div>
            ...
            <p className="mt-2 text-slate-500">Escolha o grupo de música litúrgica.</p>
          </div>
          <Link to="/grupos/novo" className="rounded-xl bg-violet-700 text-white px-5 py-3 font-semibold">Novo grupo</Link>
        </div>

        {erro && <div className="mb-5 bg-red-50 text-red-700 p-4 rounded-xl">{erro}</div>}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {grupos.map(grupo => (
            ...
            >
              ...
              <span className="inline-block mt-4 rounded-full bg-violet-50 text-violet-700 px-3 py-1 text-xs font-bold">{grupo.papel}</span>
            </button>
          ))}
        </div>
      </section>
